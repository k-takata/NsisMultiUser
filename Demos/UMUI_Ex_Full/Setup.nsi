...
!define COMMENTS "NsisMultiUser UMUI Full Demo, based on the Ultra Modern and ModernUIEx User Interfaces, using the standard MUI dialogs" ; stored as comments in the uninstall info of the registry
!define URL_INFO_ABOUT "https://github.com/Drizin/NsisMultiUser/tree/master/Demos/UMUI_Ex_Full" ; stored as the Support Link in the uninstall info of the registry, and when not included, the Help Link as well
...
Function PageComponentsShow
	; UMUI hides the Back button on the Components page for the inner instance, which makes sense, as this is the 1st page,
	; but we need to show it to return to the outer instance
